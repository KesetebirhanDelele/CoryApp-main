...
import { organizationApi, integrationApi } from '@/lib/api'
...
export default function SettingsPage() {
  ...
  const fetchTeamMembers = async () => {
    ...
    try {
      const data = await organizationApi.getTeamMembers(organization.id)
      setTeamMembers(data || [])
    } catch (error) {
  ...
  const handleSaveOrganization = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: validationError.errors?.[0]?.message || "Please check your organization data.",
        variant: "destructive",
      })
      return
    }
